...
def download_all_sources():
    ...
    with open(MASTER_RULE, "w", encoding="utf-8") as f:
        f.write("\n".join(sorted(merged)))
    unified_skip_remove()  # 下载完成后统一剔除跳过验证
    return True
...
def dns_validate(lines, part_number=0):
        ...
        with ThreadPoolExecutor(max_workers=DNS_WORKERS) as executor:
            ...
            for future in as_completed(futures):
                ...
                if done % 50 == 0 or done == len(batch):
                    ...
                    remaining = (len(lines) - (i + done)) / speed if speed > 0 else 0
                    print(f"✅ 已验证 {i + done}/{len(lines)} 条 | 有效 {len(valid)} 条 | 速度 {speed:.1f} 条/秒 | ETA {remaining:.1f} 秒")
    print(f"✅ 分片 {part_number} 验证完成，总有效 {len(valid)} 条")
    return valid

# ===============================
# 核心处理分片逻辑（更新而非替换，恢复规则排最后）
# ===============================
def process_part(part):
    ...
    # 恢复验证规则尽量排到最后一个分片
    rules_to_validate.extend(recovered_rules)

    valid = set(dns_validate(rules_to_validate, part_number=part))

    # 更新 validated_part_*.txt 并处理连续未写入删除
    for rule in rules_to_validate:
        if rule in valid:
            final_rules.add(rule)
            delete_counter[rule] = 0
            miss_write_counter[rule] = 0
            ...
            if cnt >= DELETE_THRESHOLD:
                ...
                continue
            final_rules.add(rule)
            miss_write_counter[rule] = miss_write_counter.get(rule, 0) + 1
            if miss_write_counter[rule] >= MAX_MISS_WRITE:
                print(f"🗑 连续三次未出现，删除规则：{rule}")
