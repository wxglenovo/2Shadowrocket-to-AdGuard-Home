...
DELETE_THRESHOLD = 4  # 连续 4 次才真正删除
...
def process_part(part):
    ...
    if not os.path.exists(part_file):
        print(f"⚠ 分片 {part_file} 缺失，重新下载并切片")
    ...
    final_rules = set()
    removed_count = 0
    updated_rules = set(valid) | old_rules
    ...
    valid_set = set(valid)
    print(f"✅ 分片 {part} 完成: 总 {len(final_rules)+removed_count}, 新增 {len(valid_set - old_rules)}, 删除 {removed_count}")
