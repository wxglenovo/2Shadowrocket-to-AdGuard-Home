...
# 🔹 修改说明：这里改为“更新计数文件”，用于记录连续验证失败次数，而不是删除
DELETE_THRESHOLD = 4  # 连续验证失败几次后，规则才会被暂时不保留
...
def process_part(part):
    ...
    print(f"✅ 分片 {part} 完成: 总 {len(final_rules)}, 新增 {added_count}, 超过阈值未保留 {removed_count}")
    print(f"COMMIT_STATS: 总 {len(final_rules)}, 新增 {added_count}, 超过阈值未保留 {removed_count}")
